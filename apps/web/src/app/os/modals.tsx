 ...
 *  Faithful to the prototype overlays; submit/connect actions flash a toast. */
...
const PROGS = ["솔로천국 시즌4", "환승로그", "심야 다큐", "트롯 대잔치"];

/* ─────────── UPLOAD ─────────── */
export function UploadModal({ onClose, flash, defaultProg }: { onClose: () => void; flash: (m: string) => void; defaultProg?: string }) {
  const [mode, setMode] = useState<"file" | "yt">("file");
  const [prog, setProg] = useState(defaultProg ?? PROGS[0]);
    ...
    <Overlay onClose={onClose}>
      <div className="w-[460px] max-w-full overflow-hidden rounded-2xl border border-[#2b2b2b] bg-[#131313] shadow-[0_24px_60px_rgba(0,0,0,.5)]">
        ...
        <div className="flex flex-col gap-4 p-5">
          ...
          <div><div className={labelCls}>프로그램</div><select value={prog} onChange={(e) => setProg(e.target.value)} className={inputCls}>{PROGS.map((p) => <option key={p} value={p}>{p}</option>)}</select></div>
          {mode === "file" ? (
            <>
              <div className="flex flex-col items-center justify-center gap-2 rounded-[12px] border-2 border-dashed border-[#2b2b2b] px-4 py-7 text-center">
                ...
                <div className="text-[13px] text-[#9a9a9a]">영상 파일을 끌어다 놓거나 <span className="font-semibold text-[#8b93ff]">클릭해서 선택</span><div className="mt-0.5 text-[11px] text-[#5a5a5a]">mp4 · mov · webm · 길이 제한 없음</div></div>
              </div>
              <div><div className={labelCls}>제목</div><input placeholder="비우면 파일명 사용" className={inputCls} /></div>
            </>
          ) : (
            <>
              <div><div className={labelCls}>YouTube URL</div><input placeholder="https://www.youtube.com/watch?v=…" className={inputCls} /></div>
          ...
          <button onClick={() => { onClose(); flash(`${prog} · 업로드 시작 → AI 분석 대기열`); }} className="w-full rounded-[9px] bg-[#6b74f0] py-2.5 text-[13px] font-semibold text-white hover:bg-[#5a63e6]">업로드</button>
        </div>
      </div>
    </Overlay>
  );
...
const AGES = ["전체", "7세", "12세", "15세", "19세"];
const CATS = ["드라마", "예능", "교양"];
const WEEK = ["월", "화", "수", "목", "금", "토", "일"];
export function NewProgramModal({ onClose, flash }: { onClose: () => void; flash: (m: string) => void }) {
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="flex max-h-[90vh] w-[500px] max-w-full flex-col overflow-auto rounded-2xl border border-[#2b2b2b] bg-[#131313] shadow-[0_24px_60px_rgba(0,0,0,.5)]">
        ...
        <div className="flex flex-col gap-4 p-5">
          <div><div className={labelCls}>프로그램 제목</div><input placeholder="예: 전지적 참견 시점" className={inputCls} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><div className={labelCls}>장르</div><select className={inputCls}>{GENRES.map((g) => <option key={g}>{g}</option>)}</select></div>
            <div><div className={labelCls}>시청 등급</div><select className={inputCls}>{AGES.map((a) => <option key={a}>{a}</option>)}</select></div>
          </div>
          <div><div className={labelCls}>출연자 <span className="font-normal text-[#5a5a5a]">· 쉼표로 구분 · 선택</span></div><input placeholder="이영자, 홍현희" className={inputCls} /></div>
          <div className="border-t border-[#232323] pt-4">
            ...
            <div className="mb-3.5 mt-3.5 grid grid-cols-2 gap-3">
              <div><div className={labelCls}>프로그램 코드</div><input placeholder="jamsi" className={inputCls} /></div>
        ...
        <div className="flex justify-end gap-2 border-t border-[#232323] px-5 py-3.5">
          ...
          <button onClick={() => { onClose(); flash("프로그램 생성됨 (데모)"); }} className="flex items-center gap-1.5 rounded-[9px] bg-[#6b74f0] px-4 py-2 text-[12.5px] font-semibold text-white hover:bg-[#5a63e6]"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.4}><path d="M12 5v14M5 12h14" /></svg>만들기</button>
        </div>
      </div>
    </Overlay>
  );
